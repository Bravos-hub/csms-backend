...
export class ProviderDocumentsService {
  ...
  private mapDocument(
    document:
      | ProviderDocumentWithRelationship
      | Prisma.ProviderDocumentGetPayload<Record<string, never>>,
  ) {
    return {
      id: document.id,
  ...
  private async resolveDocumentScope(
    ...
    if (relationshipId) {
      ...
      if (providerId && providerId !== relationship.providerId) {
        throw new BadRequestException(
          'providerId does not match relationship providerId',
        );
      }
    ...
    if (!providerId && !relationshipId) {
      throw new BadRequestException(
        'Either providerId or relationshipId is required',
      );
    }

    if (isProvider) {
      if (!actor.providerId)
        throw new ForbiddenException('Provider user has no providerId scope');
      if (!providerId || providerId !== actor.providerId) {
        throw new ForbiddenException(
          'providerId is outside your authenticated scope',
        );
      }
    }
    ...
    if (isOwner) {
      if (!actor.organizationId)
        throw new ForbiddenException('Owner user has no organizationId scope');
  ...
  async listDocuments(query: ProviderDocumentsQueryDto, actorId?: string) {
    ...
    if (query.my) {
      ...
      if (this.authz.isProviderRole(actor.role)) {
        ...
        throw new ForbiddenException(
          'You do not have access to provider documents',
        );
      }
    }
    ...
    const docs = await this.prisma.providerDocument.findMany({
      ...
      include: {
        relationship: {
          select: { id: true, ownerOrgId: true, providerId: true },
        },
      },
  ...
  async createDocument(data: CreateProviderDocumentDto, actorId?: string) {
    ...
    if (!isPlatform && !isProvider && !isOwner) {
      throw new ForbiddenException(
        'You do not have permission to upload provider documents',
      );
    }
  ...
  async uploadDocument(
    ...
    data: UploadProviderDocumentDto,
    actorId?: string,
  ) {
    ...
    if (!this.authz.isPlatformOps(actor.role)) {
      if (this.authz.isProviderRole(actor.role)) {
        if (!actor.providerId || doc.providerId !== actor.providerId) {
          throw new ForbiddenException(
            'Document is outside your authenticated provider scope',
          );
        }
      } else if (this.authz.isOwnerRole(actor.role)) {
        if (!actor.organizationId)
          throw new ForbiddenException(
            'Owner user has no organizationId scope',
          );
        const orgScoped =
          doc.ownerOrgId === actor.organizationId ||
          doc.relationship?.ownerOrgId === actor.organizationId;
        if (!orgScoped) {
          throw new ForbiddenException(
            'Document is outside your authenticated organization scope',
          );
        }
      } else {
        throw new ForbiddenException(
          'You do not have permission to delete provider documents',
        );
      }
    }
  ...
  async reviewDocument(
    id: string,
    data: ReviewProviderDocumentDto,
    actorId?: string,
  ) {
    const actor = await this.authz.getActor(actorId);
    this.authz.requirePlatformOps(actor);

    const existing = await this.prisma.providerDocument.findUnique({
      where: { id },
    });
    ...
    if (
      ...
      throw new BadRequestException(
        'rejectionReason or reviewNotes is required when rejecting a document',
      );
    }
