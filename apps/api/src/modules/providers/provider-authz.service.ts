...
export class ProviderAuthzService {
  ...
  private readonly ownerRoles = new Set<UserRole>([
  ...
  requirePlatformOps(actor: ProviderActor) {
    if (!this.isPlatformOps(actor.role)) {
      throw new ForbiddenException(
        'This action is restricted to platform operations roles',
      );
    }
  }
  ...
  requireProviderRole(actor: ProviderActor) {
    if (!this.isProviderRole(actor.role)) {
      throw new ForbiddenException(
        'This action is restricted to provider roles',
      );
    }
  }

  resolveOwnerOrgScope(
    actor: ProviderActor,
    requestedOwnerOrgId?: string,
  ): string {
    if (this.isPlatformOps(actor.role)) {
      if (!requestedOwnerOrgId) {
        throw new BadRequestException(
          'ownerOrgId is required for this request',
        );
      }
    ...
    if (requestedOwnerOrgId && requestedOwnerOrgId !== actor.organizationId) {
      throw new ForbiddenException(
        'ownerOrgId does not match your authenticated organization scope',
      );
    }

    if (!this.isOwnerRole(actor.role)) {
      throw new ForbiddenException(
        'Only station owner/operator users can act on owner organization scope',
      );
    }
  ...
  assertProviderScope(actor: ProviderActor, providerId: string) {
    ...
    if (!this.isProviderRole(actor.role)) {
      throw new ForbiddenException(
        'Only platform ops or provider users can perform this action',
      );
    }
    if (!actor.providerId || actor.providerId !== providerId) {
      throw new ForbiddenException(
        'Requested provider is outside your authenticated scope',
      );
    }
  }
  ...
  assertOwnerOrgScope(actor: ProviderActor, ownerOrgId: string) {
    ...
    if (!this.isOwnerRole(actor.role)) {
      throw new ForbiddenException(
        'Only station owner/operator users can perform this action',
      );
    }
    if (!actor.organizationId || actor.organizationId !== ownerOrgId) {
      throw new ForbiddenException(
        'Requested owner organization is outside your authenticated scope',
      );
    }
  }

  assertRelationshipScopedAccess(
    actor: ProviderActor,
    relationship: { providerId: string; ownerOrgId: string },
  ) {
    if (this.isPlatformOps(actor.role)) return;
    if (
      this.isProviderRole(actor.role) &&
      actor.providerId === relationship.providerId
    )
      return;
    if (
      this.isOwnerRole(actor.role) &&
      actor.organizationId === relationship.ownerOrgId
    )
      return;
    throw new ForbiddenException(
      'Relationship is outside your authenticated scope',
    );
  }
}
