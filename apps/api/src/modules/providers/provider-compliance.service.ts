...
function getProfileStringArray(
  profile: Record<string, unknown>,
  key: string,
): string[] {
  const raw = profile[key];
...
function evaluateCondition(
  ...
  if (condition.key === 'crossBorderShipping') {
    const inferredCrossBorder =
      (provider.countries?.length || 0) > 1 ||
      (provider.regions?.length || 0) > 1;
    if (condition.operator === 'eq')
      return inferredCrossBorder === Boolean(condition.value);
  ...
  if (condition.key === 'supportsInteroperability') {
    ...
    if (condition.operator === 'eq')
      return isInterop === Boolean(condition.value);
    return isInterop;
  }

  if (condition.key === 'supportsAirShipping') {
    const supportsAirShipping = getProfileBoolean(
      profile,
      'supportsAirShipping',
    );
    if (condition.operator === 'eq')
      return supportsAirShipping === Boolean(condition.value);
...
function requirementMatchesMarket(
  requirement: ProviderRequirementDefinition,
  activeMarkets: Array<'CN' | 'HK' | 'FI'>,
): boolean {
  if (requirement.markets.includes('GLOBAL')) return true;
  return requirement.markets.some((market) =>
    activeMarkets.includes(market as 'CN' | 'HK' | 'FI'),
  );
}
...
export class ProviderComplianceService {
  ...
  private evaluateRequirementApplies(options: {
    ...
    const { requirement, provider, profile, markets, policy, policyWarnings } =
      options;
    ...
    if (requirement.policyDependency === 'HK_DG_THRESHOLD') {
      ...
      const dgClassifications = getProfileStringArray(
        profile,
        'dgClassifications',
      ).map((item) => item.toUpperCase());
      const class9aLabel = (
        policy.hk.dg.class9aLabel || 'DG_CLASS_9A'
      ).toUpperCase();

      const triggeredByEnergy =
        typeof threshold === 'number' &&
        storedEnergyKwh != null &&
        storedEnergyKwh >= threshold;
    ...
    return requirement.conditions.every((condition) =>
      evaluateCondition(condition, provider, profile),
    );
  }
  ...
  private evaluateRequirementSatisfaction(options: {
    ...
    const matchingDocs = documents.filter((doc) => {
      ...
      if (doc.requirementCode)
        return doc.requirementCode === requirement.requirementCode;
    ...
    const missingModel = providerModels.find(
      (model) => (coveredByModel.get(model) || []).length === 0,
    );
    if (missingModel) {
      return { satisfied: false, matchedDocumentIds: [] };
    }
  ...
  private buildCompliance(options: {
    ...
    const relationshipMarkets = relationship
      ? mapComplianceMarkets({
          complianceMarkets: relationship.complianceMarkets,
        })
      : [];
    const inferredMarkets = Array.from(
      new Set([...(providerMarkets || []), ...(relationshipMarkets || [])]),
    );
    const activeMarkets = filterActiveMarketsByPolicy(
      inferredMarkets,
      policy.markets,
    );
    ...
    const relationshipProfile = relationship
      ? asObject(relationship.complianceProfile)
      : {};
    const mergedProfile = { ...providerProfile, ...relationshipProfile };

    const providerModels = (provider.batteriesSupported || [])
    ...
    const evaluations: RequirementEval[] = requirements.map((requirement) => {
      ...
      const downgradedPreEffective =
        ...
        (requirement.roadmapAllowedBeforeEffective ??
          policy.roadmapAllowedBeforeEffective);

      const { satisfied, matchedDocumentIds } =
        this.evaluateRequirementSatisfaction({
          requirement,
          documents,
          providerModels,
        });
    ...
    const gateStatuses: GateStatus[] = GATE_ORDER.map((gate) => {
      const gateItems = activeEvaluations.filter(
        (item) => item.requirement.gate === gate,
      );
      const missingCritical = gateItems
        .filter(
          (item) =>
            !item.satisfied &&
            item.requirement.isCritical &&
            !item.downgradedPreEffective,
        )
      ...
      const missingRecommended = gateItems
        .filter(
          (item) =>
            !item.satisfied &&
            (!item.requirement.isCritical || item.downgradedPreEffective),
        )
        .map((item) => item.requirement.requirementCode);

      const met = gateItems.filter((item) => item.satisfied).length;
      const criticalRequired = gateItems.filter(
        (item) => item.requirement.isCritical && !item.downgradedPreEffective,
      ).length;
      const criticalMet = gateItems.filter(
        (item) =>
          item.satisfied &&
          item.requirement.isCritical &&
          !item.downgradedPreEffective,
    ...
    const missingCritical = Array.from(
      new Set(gateStatuses.flatMap((item) => item.missingCritical)),
    );
    const missingRecommended = Array.from(
      new Set(gateStatuses.flatMap((item) => item.missingRecommended)),
    );
    ...
    const expiringSoon = documents.filter(
      (item) => isDocumentVerified(item) && isExpiringSoon(item, now),
    );
    const expiredCritical = documents.filter((item) => {
      ...
      const matchedRequirement = activeEvaluations.find((evaluation) => {
        if (
          !matchedCriticalRequirementCodes.has(
            evaluation.requirement.requirementCode,
          )
        )
          return false;
        if (item.requirementCode)
          return (
            item.requirementCode === evaluation.requirement.requirementCode
          );
  ...
  private async assertProviderComplianceAccess(
    providerId: string,
    actorId?: string,
  ) {
    ...
    if (this.authz.isOwnerRole(actor.role)) {
      ...
      const scopedRelationship =
        await this.prisma.providerRelationship.findFirst({
          where: {
            providerId,
      ...
      if (!scopedRelationship) {
        throw new ForbiddenException(
          'Provider is outside your authenticated organization scope',
        );
      }
      return;
    }
    ...
    if (!this.authz.isPlatformOps(actor.role)) {
      throw new ForbiddenException(
        'You do not have access to provider compliance status',
      );
    }
  }
  ...
  async getProviderComplianceStatus(providerId: string, actorId?: string) {
    ...
    const requirements =
      this.providerRequirementsService.listForScope('PROVIDER');
  ...
  async getProviderComplianceStatuses(providerIds: string[], actorId?: string) {
    ...
    return Promise.all(
      unique.map((providerId) =>
        this.getProviderComplianceStatus(providerId, actorId),
      ),
    );
  }

  async getRelationshipComplianceStatus(
    relationshipId: string,
    actorId?: string,
  ) {
    const actor = await this.authz.getActor(actorId);
    const relationship =
      await this.prisma.providerRelationship.findUniqueOrThrow({
    ...
    const requirements =
      this.providerRequirementsService.listForScope('STATION_OWNER');
    return this.buildCompliance({
      scope: 'RELATIONSHIP',
  ...
  async getRelationshipComplianceStatuses(
    ...
    if (unique.length === 0) return [];
    return Promise.all(
      unique.map((relationshipId) =>
        this.getRelationshipComplianceStatus(relationshipId, actorId),
      ),
    );
  }
}
