...
export class ProviderCompliancePolicyService {
  ...
  normalizePolicy(data: unknown): ProviderCompliancePolicyData {
    ...
    const effectiveDateMode =
      root.effectiveDateMode === 'ENFORCE_NOW'
        ? 'ENFORCE_NOW'
        : 'WARN_BEFORE_ENFORCE';
    const roadmapAllowedBeforeEffective =
      root.roadmapAllowedBeforeEffective !== false;

    const marketsRaw = Array.isArray(root.markets)
      ? root.markets.map((item) => String(item).toUpperCase())
      : DEFAULT_POLICY.markets;
    const markets = marketsRaw.filter(
      (item): item is 'CN' | 'HK' | 'FI' =>
        item === 'CN' || item === 'HK' || item === 'FI',
    );
    ...
    return {
      ...
      hk: {
        dg: {
          ...
          thresholdKwh,
          class9aLabel:
            typeof dg.class9aLabel === 'string' &&
            dg.class9aLabel.trim().length > 0
  ...
  async getProviderPolicy() {
    const existing = await this.prisma.compliancePolicy.findUnique({
  ...
  async updateProviderPolicy(
    input: Partial<ProviderCompliancePolicyData>,
    updatedBy?: string,
  ) {
