...
async function checkPostGIS() {
  ...
  try {
    ...
    if (!hasPostGIS) {
      console.log(
        'PostGIS not found in extensions. Attempting to check for ST_AsMVT function...',
      );
      const funcRes = await client.query(
        "SELECT routine_name FROM information_schema.routines WHERE routine_name = 'st_asmvt'",
      );
