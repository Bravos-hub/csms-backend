...
async function main() {
  console.log(
    'Starting migration for legacy STATION_OWNER users without an organization...',
  );
  ...
  if (legacyUsers.length === 0) {
    console.log(
      'No legacy STATION_OWNER users found requiring migration. Exiting.',
    );
    return;
  }
  ...
  for (const user of legacyUsers) {
    ...
    const orgName = user.name
      ? `${user.name} LLC`
      : `${user.email?.split('@')[0] || user.id} LLC`;
    ...
    try {
      ...
      console.log(
        `✅ Success: User ${user.id} linked to Organization ${org.id} (${orgName})`,
      );
    } catch (e) {
